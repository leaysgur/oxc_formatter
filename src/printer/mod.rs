...
use crate::{FormatElement, GroupId, IndentStyle, PrintResult, Printed};

use crate::format_element::document::Document;
use crate::format_element::tag::Condition;
use crate::prelude::Tag::EndFill;
use crate::prelude::tag::{DedentMode, Tag, TagKind};
    ...
    fn fits_element(&mut self, element: &'a FormatElement) -> PrintResult<Fits> {
        ...
        match element {
            ...
            FormatElement::Tag(StartGroup(group)) => {
                ...
                let group_mode =
                    if !group.mode().is_flat() { PrintMode::Expanded } else { args.mode() };
...
fn invalid_end_tag<R>(end_tag: TagKind, start_tag: Option<TagKind>) -> PrintResult<R> {
    Err(match start_tag {
        None => format!("Expected start tag {end_tag:?} but got EndOfDocument"),
        Some(kind) => {
            format!("Expected end tag {end_tag:?} but got {kind:?}")
        }
    })
}
...
fn invalid_start_tag<R>(expected: TagKind, actual: Option<&FormatElement>) -> PrintResult<R> {
    ...
    Err(format!("Expected start tag {expected:?} but got {start:?}",))
}
