...
use crate::base_formatter::{CstFormatContext, FormatResult, FormatRule, TextSize, TransformSourceMap};
use crate::base_formatter::{buffer::Buffer, write};
...
pub struct Comments<L: Language> {
    /// The use of a [Rc] is necessary to achieve that [Comments] has a lifetime that is independent from the [crate::base_formatter::Formatter].
    /// Having independent lifetimes is necessary to support the use case where a (formattable object)[crate::base_formatter::Format]
    /// iterates over all comments, and writes them into the [crate::base_formatter::Formatter] (mutably borrowing the [crate::base_formatter::Formatter] and in turn its context).
    ///
    /// ```block
